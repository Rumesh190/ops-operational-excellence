...
import { ArrowLeft, Building2, CalendarDays, Camera, Check, CheckCircle2, ClipboardCheck, ExternalLink, FileText, History, IndianRupee, Link2, MapPin, Maximize2, Paperclip, Play, RotateCcw, Send, Upload, UserRound, X } from "lucide-react";
...
export default function FiveSActionDetailPage({ actionId, initialSection }: { actionId: string; initialSection?: string }) {
  ...
  const progressCameraInputRef = useRef<HTMLInputElement | null>(null);
  const completionCameraInputRef = useRef<HTMLInputElement | null>(null);
  ...
  return <PageContainer className="max-w-none">
    ...
    {section === "evidence" && <div className="grid min-w-0 gap-5 xl:grid-cols-3"><EvidencePanel tone="before" title="Before Evidence" description="Original condition captured when this action was created." evidence={action.issueEvidence ?? []} onPreview={setPreview} /><EvidencePanel tone="progress" title="Progress Evidence" description="Optional evidence captured while corrective work is underway." evidence={action.progressEvidence ?? []} editable={canEdit && !canStart} onUpload={() => progressInputRef.current?.click()} onTakePhoto={() => progressCameraInputRef.current?.click()} onPreview={setPreview} onRemove={(id) => removeActionProgressEvidence(action.id, id)} /><EvidencePanel tone="completion" title="Completion Evidence" description="Evidence supporting completion and reviewer verification." evidence={action.evidence} editable={canEdit && !canStart} required onUpload={() => completionInputRef.current?.click()} onTakePhoto={() => completionCameraInputRef.current?.click()} onPreview={setPreview} onRemove={(id) => removeActionEvidence(action.id, id)} /><input ref={progressInputRef} hidden type="file" accept="image/*,.pdf,.doc,.docx" onChange={(event) => void addEvidence(event, "progress")} /><input ref={progressCameraInputRef} hidden type="file" accept="image/*" capture="environment" onChange={(event) => void addEvidence(event, "progress")} /><input ref={completionInputRef} hidden type="file" accept="image/*,.pdf,.doc,.docx" onChange={(event) => void addEvidence(event, "completion")} /><input ref={completionCameraInputRef} hidden type="file" accept="image/*" capture="environment" onChange={(event) => void addEvidence(event, "completion")} /></div>}
...
function EvidencePanel({ tone, title, description, evidence, editable = false, required = false, compact = false, onUpload, onTakePhoto, onPreview, onRemove = () => undefined }: { tone: "before" | "progress" | "completion"; title: string; description: string; evidence: MyActionEvidence[]; editable?: boolean; required?: boolean; compact?: boolean; onUpload?: () => void; onTakePhoto?: () => void; onPreview: (item: MyActionEvidence) => void; onRemove?: (id: string) => void }) {
  const tones = { before: "bg-red-500", progress: "bg-amber-500", completion: "bg-emerald-500" };
  return <Card className="min-w-0 gap-0 overflow-hidden"><CardHeader className="border-b pb-4"><div className="flex flex-col gap-3"><div><div className="flex items-center gap-2"><span className={cn("size-2 rounded-full", tones[tone])} /><CardTitle className="text-sm">{title}{required && <span className="text-destructive"> *</span>}</CardTitle></div><p className="mt-1 text-xs leading-5 text-muted-foreground">{description}</p></div>{editable && <div className="grid grid-cols-2 gap-2">{onTakePhoto && <Button size="sm" variant="outline" onClick={onTakePhoto}><Camera className="size-3.5" />Take Photo</Button>}{onUpload && <Button size="sm" variant="outline" onClick={onUpload}><Upload className="size-3.5" />Upload Photo</Button>}</div>}</div></CardHeader><CardContent className="p-4"><EvidenceGrid evidence={evidence} editable={editable} compact={compact} onPreview={onPreview} onRemove={onRemove} emptyLabel={`No ${title.toLowerCase()} attached`} /></CardContent></Card>;
}
