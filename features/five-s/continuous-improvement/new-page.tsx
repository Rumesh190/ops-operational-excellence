...
export default function ContinuousImprovementNewPage() {
  ...
  return <PageContainer className="max-w-5xl pb-24 sm:pb-8">
    ...
    <Card className="gap-0"><CardHeader className="border-b pb-4"><SectionHeading icon={FileEdit} eyebrow="Current State" title="What is happening today?" /></CardHeader><CardContent className="grid gap-5 p-4 sm:p-5"><Field label="Title *"><Input autoFocus value={title} onChange={(event) => setTitle(event.target.value)} maxLength={120} placeholder="Example: Reduce material movement in packing area" /></Field><div className="grid gap-4 sm:grid-cols-2"><Field label="Plant *"><Select value={currentUser.plant}><SelectTrigger className="w-full"><SelectValue /></SelectTrigger><SelectContent><SelectItem value={currentUser.plant}>{currentUser.plant}</SelectItem></SelectContent></Select></Field><Field label="Zone *"><Select value={zoneName} onValueChange={(value) => { setZoneName(value ?? zoneName); setMemberIds([]); }}><SelectTrigger className="w-full"><SelectValue /></SelectTrigger><SelectContent>{availableZones.map((item) => <SelectItem key={item.name} value={item.name}>{item.name}</SelectItem>)}</SelectContent></Select></Field><Field label="Proposed By"><Input value={currentUser.name} disabled /></Field><Field label="Owner"><Input value={currentUser.name} disabled /></Field></div><Field label="Problem / Opportunity *"><Textarea value={problem} onChange={(event) => setProblem(event.target.value)} rows={5} maxLength={1200} placeholder="Describe the current method, waste, delay, or opportunity without framing it as an incident." /></Field><div><Label className="mb-2 block">Current-state Evidence</Label><ImprovementEvidencePicker items={beforeEvidence} onChange={setBeforeEvidence} uploadedBy={currentUser.name} label="Upload evidence" /></div></CardContent></Card>
