...
function ProposalEditor({ item }: { item: ContinuousImprovement }) {
  ...
  function save(submitDraft = false) {
    setNotice("");
    try {
      updateImprovementProposal(item.id, { title, issueDescription: problem, proposedImprovement: proposal, expectedBenefit, benefitType, proposedSaving: proposedSaving === "" ? undefined : Number(proposedSaving), estimatedTime: Number(estimatedTime), estimatedTimeUnit, memberIds, beforeEvidence }, currentUser);
  ...
  return <PageContainer className="max-w-5xl pb-24 sm:pb-8"><FiveSPageHeader eyebrow="Continuous Improvement" title="Edit Proposal" description={`${item.id} · Update the opportunity, proposal, and expected benefit.`} leading={<Button variant="ghost" size="icon" nativeButton={false} render={<Link href={`/continuous-improvement/${encodeURIComponent(item.id)}`} />}><ArrowLeft className="size-4" /></Button>} actions={<ImprovementStatusBadge status={item.status} />} />{notice && <div role="status" className="rounded-lg border bg-muted/20 px-4 py-3 text-sm">{notice}</div>}
    <Card className="gap-0"><CardHeader className="border-b pb-4"><CardTitle className="text-base">Current State</CardTitle></CardHeader><CardContent className="grid gap-4 p-4 sm:p-5"><Field label="Title *"><Input value={title} onChange={(event) => setTitle(event.target.value)} /></Field><div className="grid gap-4 sm:grid-cols-2"><Field label="Plant"><Input value={item.plant} disabled /></Field><Field label="Zone"><Input value={item.zone} disabled /></Field></div><Field label="Problem / Opportunity *"><Textarea rows={5} value={problem} onChange={(event) => setProblem(event.target.value)} /></Field><div><Label className="mb-2 block">Current-state Evidence</Label><ImprovementEvidencePicker items={beforeEvidence} onChange={setBeforeEvidence} uploadedBy={currentUser.name} /></div></CardContent></Card>
