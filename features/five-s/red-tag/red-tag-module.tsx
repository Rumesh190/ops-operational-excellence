...
function RedTagCreateForm() {
  ...
  const validWithoutPhoto = Boolean(item.trim() && remarks.trim() && Number(quantity) >= 1 && location.trim() && zoneConfig?.department && reason && (reason !== "Others" || customReason.trim()) && category && (parsedValue === undefined || (Number.isFinite(parsedValue) && parsedValue >= 0)));
  async function imageChanged(e: React.ChangeEvent<HTMLInputElement>) { const file = e.target.files?.[0]; if (!file) return; try { const { dataUrl } = await optimizeEvidenceImage(file); setImageUrl(dataUrl); setPhotoError(""); } catch (error) { window.alert(error instanceof Error ? error.message : "Unable to process this image."); } e.target.value = ""; }
  function submit() { if (!imageUrl) { setPhotoError("Before photo is required to create this Red Tag."); return; } if (!validWithoutPhoto || !reason || !category || !zoneConfig || creating) return; setCreating(true); window.setTimeout(() => { try { const tag = createRedTagV2({ plant: user.plant, zone, section: location.trim(), department: zoneConfig.department, category, estimatedValue: parsedValue, itemName: item.trim(), quantity: Number(quantity), reason, customReason: customReason.trim() || undefined, remarks: remarks.trim(), createdById: user.id, createdByName: user.name, imageUrl }, user); if (!tag) { setCreating(false); return; } router.push(`/5s/red/${tag.id}`); } catch { setCreating(false); } }, 240); }
  return <PageContainer className="max-w-none">
    <FiveSPageHeader eyebrow="Red Tags / Create" title="Create Red Tag" description="Identify a physical item and capture the context needed for review."
      ...
      actions={<><Button variant="ghost" onClick={() => router.push("/5s/red")} disabled={creating}>{t("common.cancel")}</Button><Button disabled={!validWithoutPhoto || creating} onClick={submit}><Plus className="size-4" /> {creating ? "Creating Tag..." : t("redTag.create")}</Button></>} />
    <form onSubmit={(e) => { e.preventDefault(); submit(); }} className="grid gap-5">
      ...
      <Card><CardContent className="grid gap-5 p-5"><div><h2 className="font-semibold">Before / Identification Photo</h2><p className="mt-1 text-xs text-muted-foreground">A Before photo of the physical item or condition is required.</p></div>
        <Field label="Before Photo *"><div className="grid gap-3"><div className="grid grid-cols-2 gap-2 sm:flex"><Button type="button" variant="outline" onClick={() => cameraRef.current?.click()}><Camera className="size-4" />Take Photo</Button><Button type="button" variant="outline" onClick={() => uploadRef.current?.click()}><Upload className="size-4" />Upload Photo</Button></div><input ref={uploadRef} hidden type="file" accept="image/*" onChange={imageChanged} /><input ref={cameraRef} hidden type="file" accept="image/*" capture="environment" onChange={imageChanged} />{imageUrl && <div className="flex flex-wrap items-center gap-2 rounded-lg border p-2"><button type="button" onClick={() => setPreview(true)}><img src={imageUrl} alt="Before item preview" className="size-20 rounded object-cover" /></button><Button type="button" size="sm" variant="ghost" onClick={() => setPreview(true)}>View</Button><Button type="button" size="sm" variant="ghost" onClick={() => uploadRef.current?.click()}>Replace</Button><Button type="button" size="sm" variant="ghost" className="text-destructive" onClick={() => { setImageUrl(undefined); setPhotoError("Before photo is required to create this Red Tag."); }}><Trash2 className="size-4" />Remove</Button></div>}{photoError && <p role="alert" className="text-sm text-destructive">{photoError}</p>}</div></Field>
      </CardContent></Card>
      <div className="flex justify-end sm:hidden"><Button className="w-full" disabled={!validWithoutPhoto || creating} type="submit"><Plus className="size-4" /> {creating ? "Creating Tag..." : "Create Red Tag"}</Button></div>
    </form>
...
export function RedTagDetailPage({ tagId }: { tagId: string }) {
  ...
  return <PageContainer className="max-w-none"><FiveSPageHeader eyebrow="Red Tags / Details" title={tag.tagNumber} description={`${tag.itemName} · ${tag.section}`}
    ...
    <div className="grid gap-5 xl:grid-cols-[minmax(0,1.7fr)_minmax(320px,.7fr)]"><div className="grid gap-5">
      ...
      <Card><CardContent className="p-5"><h2 className="font-semibold">Before / After Evidence</h2><div className="mt-4 grid gap-4 sm:grid-cols-2"><div><p className="text-xs font-semibold uppercase tracking-wide text-muted-foreground">Before</p>{tag.imageUrl ? <Image unoptimized width={640} height={480} src={tag.imageUrl} alt={`Before · ${tag.itemName}`} className="mt-2 aspect-[4/3] w-full rounded-lg border object-contain" /> : <div className="mt-2 grid aspect-[4/3] place-items-center rounded-lg border border-dashed text-xs text-muted-foreground"><span><ImageIcon className="mx-auto mb-2 size-6" />Before photo unavailable</span></div>}</div><div><p className="text-xs font-semibold uppercase tracking-wide text-muted-foreground">After</p>{tag.afterEvidence?.[0] ? <Image unoptimized width={640} height={480} src={tag.afterEvidence[0].url} alt={`After · ${tag.itemName}`} className="mt-2 aspect-[4/3] w-full rounded-lg border object-contain" /> : <div className="mt-2 grid aspect-[4/3] place-items-center rounded-lg border border-dashed text-xs text-muted-foreground">Pending disposition</div>}</div></div></CardContent></Card>
...
function RedTagV2VerificationSection({ tag, action, mayManage, user }: { tag: RedTag; action?: ReturnType<typeof useActionStore>[number]; mayManage: boolean; user: ReturnType<typeof useCurrentUser> }) {
  ...
  function verify() {
    ...
    if (result === "Passed" && !tag.imageUrl?.trim()) { setError("Before photo evidence is required before verification can pass."); return; }
    if (result === "Passed" && !(tag.afterEvidence?.length || evidence.length)) { setError("After photo evidence is required before verification can pass."); return; }
  ...
  function close() {
    if (!closeRedTag(tag.id, user)) setError("Before and after photo evidence plus successful verification are required before closure.");
    else setError("");
  }
...
function AfterEvidencePicker({ items, onChange, userName }: { items: RedTagEvidence[]; onChange: (items: RedTagEvidence[]) => void; userName: string }) {
  ...
  return <Field label="After Photo *"><div className="grid gap-3"><div className="grid grid-cols-2 gap-2 sm:flex"><Button type="button" variant="outline" onClick={() => cameraRef.current?.click()}><Camera className="size-4" />Take Photo</Button><Button type="button" variant="outline" onClick={() => uploadRef.current?.click()}><Upload className="size-4" />Upload Photo</Button></div><input ref={uploadRef} hidden multiple type="file" accept="image/*" onChange={changed} /><input ref={cameraRef} hidden type="file" accept="image/*" capture="environment" onChange={changed} />{items.length > 0 && <div className="grid grid-cols-2 gap-2 sm:grid-cols-4">{items.map((item) => <div key={item.id} className="overflow-hidden rounded-lg border"><Image unoptimized width={160} height={120} src={item.url} alt={item.name} className="aspect-[4/3] w-full object-cover" /><div className="border-t"><button type="button" className="min-h-9 w-full text-xs font-medium text-destructive" onClick={() => onChange(items.filter((candidate) => candidate.id !== item.id))}>Remove</button></div></div>)}</div>}</div></Field>;
}
