...
export function VisualEvidencePicker({ items, onChange, uploadedBy, label = "Add photos", required = false }: {
  ...
  return <div className="grid gap-3">
    ...
    <div className="grid gap-2 sm:grid-cols-[minmax(0,1fr)_auto_auto]">
      ...
      <Button type="button" variant="outline" className="min-h-11" onClick={() => cameraRef.current?.click()} disabled={busy || items.length >= MAX_EVIDENCE_IMAGES}><Camera className="size-4" />Camera</Button>
      <Button type="button" variant="outline" className="min-h-11" onClick={() => uploadRef.current?.click()} disabled={busy || items.length >= MAX_EVIDENCE_IMAGES}><Upload className="size-4" />{busy ? "Processing..." : label}</Button>
    </div>
