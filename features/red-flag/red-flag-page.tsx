...
import { ACTIVE_RED_FLAG_SEVERITIES, type RedFlag, type RedFlagSeverity } from "./types";
...
function RedFlagOverview({ flags, actions }: { flags: RedFlag[]; actions: MyAction[] }) {
  ...
  const severityValues: RedFlagSeverity[] = open.some((flag) => flag.severity === "Critical") ? ["Critical", ...ACTIVE_RED_FLAG_SEVERITIES] : [...ACTIVE_RED_FLAG_SEVERITIES];
  const severity = severityValues.map((item) => ({ label: item, value: open.filter((flag) => flag.severity === item).length }));
...
function RedFlagLibrary({ flags, actions, tab, initialStatus, initialSeverity }: { flags: RedFlag[]; actions: MyAction[]; tab: Exclude<LandingTab, "overview">; initialStatus?: string; initialSeverity?: string }) {
  const hasLegacyCritical = flags.some((flag) => flag.severity === "Critical");
  const [search, setSearch] = useState("");
  const [severity, setSeverity] = useState(initialSeverity && [...ACTIVE_RED_FLAG_SEVERITIES, ...(hasLegacyCritical ? ["Critical"] : [])].includes(initialSeverity) ? initialSeverity : "All");
