...
import type { RedFlag, RedFlagEvidence, RedFlagSeverity } from "./types";
...
export default function RedFlagNewPage() {
  ...
  const [severity, setSeverity] = useState<RedFlagSeverity | "">("");
  ...
  return <PageContainer className="red-flag-quick-capture max-w-4xl pb-20 sm:pb-0">
    ...
    <Card className="gap-0 overflow-hidden border-red-500/15 dark:border-red-400/15">
      ...
      <CardContent className="grid gap-5 p-4 sm:p-5">
        ...
        <div><p className="mb-2 text-sm font-medium">Severity *</p><div className="grid grid-cols-2 gap-2 sm:grid-cols-4">{(["Critical", "High", "Medium", "Low"] as RedFlagSeverity[]).map((item) => <button key={item} type="button" onClick={() => setSeverity(item)} className={cn("flex min-h-11 items-center gap-2 rounded-lg border px-3 text-sm font-medium outline-none transition-colors hover:bg-muted/40 focus-visible:ring-2 focus-visible:ring-ring", severity === item ? RED_FLAG_SEVERITY_CONFIG[item].soft + " border-current" : "bg-background")}><span className={cn("size-2 rounded-full", RED_FLAG_SEVERITY_CONFIG[item].dot)} />{item}</button>)}</div>{severity && <p className="mt-2 text-[11px] text-muted-foreground">Response target: {severity === "Critical" ? "8 hours" : severity === "High" ? "24 hours" : severity === "Medium" ? "72 hours" : "7 days"}.</p>}</div>
