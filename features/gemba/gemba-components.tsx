...
import { Camera, Eye, FileCheck2, ImageIcon, MapPin, Pencil, Plus } from "lucide-react";
...
import type { GembaObservation, GembaObservationType, GembaWalkStatus } from "./types";
...
export const OBSERVATION_TYPE_STYLE: Record<GembaObservationType, {
  ...
  Positive: { variant: "success", dot: "bg-emerald-500", border: "border-l-emerald-500", soft: "bg-emerald-500/[0.055]" },
  Opportunity: { variant: "warning", dot: "bg-amber-500", border: "border-l-amber-500", soft: "bg-amber-500/[0.055]" },
  Issue: { variant: "danger", dot: "bg-red-500", border: "border-l-red-500", soft: "bg-red-500/[0.055]" },
};
...
export function ObservationCard({ observation, action, onEdit, onCreateAction, compact = false }: {
  ...
  return <>
    ...
    <Dialog open={viewOpen} onOpenChange={setViewOpen}><DialogContent className="max-w-2xl"><DialogHeader><div className="flex flex-wrap items-center gap-2"><ObservationTypeBadge type={observation.type} /><span className="font-mono text-xs text-muted-foreground">{observation.id}</span></div><DialogTitle className="pt-1 text-lg leading-6">{observation.title}</DialogTitle><DialogDescription>{observation.location} · Added by {observation.createdByName}</DialogDescription></DialogHeader><p className="text-sm leading-6">{observation.description}</p>{observation.peopleInvolved.length > 0 && <p className="text-xs text-muted-foreground">People involved: {observation.peopleInvolved.join(", ")}</p>}{observation.evidence.length > 0 && <div className="grid grid-cols-2 gap-2 sm:grid-cols-3">{observation.evidence.map((item) => <button type="button" key={item.id} onClick={() => setPreview(item)} className="overflow-hidden rounded-lg border bg-muted outline-none focus-visible:ring-2 focus-visible:ring-ring"><img src={item.url} alt={item.note || item.name} className="aspect-[4/3] w-full object-cover" /><span className="block truncate px-2 py-1.5 text-left text-[11px] text-muted-foreground">{item.note || item.name}</span></button>)}</div>}</DialogContent></Dialog>
    <OpsEvidenceViewer open={Boolean(preview)} onOpenChange={(open) => !open && setPreview(null)} src={preview?.url} title={preview?.name} description={preview?.note || "Gemba observation evidence"} alt={preview?.note || preview?.name} />
  </>;
}
