...
import { Camera, Eye, FileCheck2, ImageIcon, Lightbulb, MapPin, Mic, Pencil, Plus, Share2, Tag } from "lucide-react";
...
export function ObservationCard({ observation, action, onEdit, onCreateAction, onCreateRedTag, onCreateImprovement, compact = false }: {
  ...
  return <>
    ...
    <Dialog open={viewOpen} onOpenChange={setViewOpen}><DialogContent className="max-w-3xl max-h-[90vh] overflow-y-auto"><DialogHeader><div className="flex flex-wrap items-center gap-2"><ObservationTypeBadge type={observation.type} /><span className="font-mono text-xs text-muted-foreground">{observation.id}</span></div><DialogTitle className="pt-1 text-lg leading-6">{observation.title}</DialogTitle><DialogDescription>{observation.location} · Added by {observation.createdByName}</DialogDescription></DialogHeader><div className="space-y-5"><p className="text-sm leading-6">{observation.description}</p>{observation.peopleInvolved.length > 0 && <div><p className="text-xs font-medium text-muted-foreground">People involved</p><p className="mt-1 text-sm">{observation.peopleInvolved.join(", ")}</p></div>}{observation.evidence.length > 0 && <div><p className="mb-3 text-xs font-medium text-muted-foreground">{observation.evidence.length} photo{observation.evidence.length === 1 ? "" : "s"}</p><div className="grid grid-cols-2 gap-3 sm:grid-cols-3">{observation.evidence.map((item) => <button type="button" key={item.id} onClick={() => setPreview(item)} className="overflow-hidden rounded-lg border bg-muted outline-none focus-visible:ring-2 focus-visible:ring-ring"><GembaEvidenceImage evidence={item} alt={item.note || item.name} className="aspect-[4/3] w-full object-cover" /><span className="block truncate px-2 py-1.5 text-left text-[11px] text-muted-foreground">{item.note || item.name}</span></button>)}</div></div>}{observation.voiceNote && <VoiceNotePlayback voiceNote={observation.voiceNote} />}{observation.horizontalDeployment?.enabled && <div className="rounded-lg border bg-muted/[0.14] p-4"><p className="flex items-center gap-2 text-xs font-semibold"><Share2 className="size-3.5 text-primary" />Horizontal Deployment</p><p className="mt-1 text-[11px] text-muted-foreground">Shared with {observation.horizontalDeployment.recipients.length} zone{observation.horizontalDeployment.recipients.length === 1 ? "" : "s"} for review.</p><div className="mt-3 divide-y">{observation.horizontalDeployment.recipients.map((recipient) => <div key={recipient.zoneId} className="flex items-center justify-between gap-3 py-2 text-xs"><span><span className="block font-medium">{formatGembaZoneLabel(recipient.zoneName)}</span><span className="block text-[11px] text-muted-foreground">{recipient.leaderName ? `Zone Leader · ${recipient.leaderName}` : "Zone Leader not assigned"}</span></span><Badge size="sm" variant={recipient.notificationStatus === "shared" ? "success" : "secondary"}>{recipient.notificationStatus === "shared" ? "Shared" : "Notification unavailable"}</Badge></div>)}</div></div>}{(hasAction || hasRedTag || hasImprovement) && <div className="rounded-lg border bg-muted/20 p-4"><p className="text-xs font-medium text-muted-foreground">Linked records</p><div className="mt-2 flex flex-wrap gap-2">{hasAction && <Link href={`/actions/${encodeURIComponent(action!.id)}`} className="inline-flex items-center gap-1.5 rounded-md border bg-background px-3 py-1.5 text-xs font-medium outline-none hover:bg-muted/50 focus-visible:ring-2 focus-visible:ring-ring"><FileCheck2 className="size-3.5" />{action!.id}</Link>}{hasRedTag && <Link href={`/5s/red/${encodeURIComponent(observation.redTagId!)}`} className="inline-flex items-center gap-1.5 rounded-md border bg-background px-3 py-1.5 text-xs font-medium outline-none hover:bg-muted/50 focus-visible:ring-2 focus-visible:ring-ring"><Tag className="size-3.5" />{observation.redTagId}</Link>}{hasImprovement && <Link href={`/continuous-improvement/${encodeURIComponent(observation.improvementId!)}`} className="inline-flex items-center gap-1.5 rounded-md border bg-background px-3 py-1.5 text-xs font-medium outline-none hover:bg-muted/50 focus-visible:ring-2 focus-visible:ring-ring"><Lightbulb className="size-3.5" />{observation.improvementId}</Link>}</div></div>}</div></DialogContent></Dialog>
