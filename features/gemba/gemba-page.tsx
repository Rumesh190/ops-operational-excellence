...
function WalkLibrary({ walks, mayConduct, onStart }: { walks: GembaWalk[]; mayConduct: boolean; onStart: (walk: GembaWalk) => void }) {
  ...
  return <Card className="min-w-0 gap-0 overflow-hidden">
    <CardContent className="grid gap-2 border-b p-3 sm:grid-cols-[minmax(240px,1fr)_180px]"><div className="relative"><Search className="absolute left-3 top-1/2 size-4 -translate-y-1/2 text-muted-foreground" /><Input className="pl-9" value={search} onChange={(event) => setSearch(event.target.value)} placeholder="Search walks, zone, or lead..." /></div><Select value={status} onValueChange={(value) => setStatus(value ?? "All")}><SelectTrigger className="w-full"><SelectValue /></SelectTrigger><SelectContent><SelectItem value="All">All statuses</SelectItem><SelectItem value="Draft">Draft</SelectItem><SelectItem value="In Progress">In Progress</SelectItem><SelectItem value="Completed">Completed</SelectItem></SelectContent></Select></CardContent>
    <div className="hidden overflow-x-auto lg:block"><table className="w-full min-w-[1100px] text-sm"><thead className="border-b bg-muted/25 text-left text-[10px] uppercase tracking-wide text-muted-foreground"><tr><th className="px-4 py-3">Walk ID</th><th className="px-4 py-3">Plant / Zone</th><th className="px-4 py-3">Lead</th><th className="px-4 py-3">Participants</th><th className="px-4 py-3">Date</th><th className="px-4 py-3">Observations</th><th className="px-4 py-3">Actions</th><th className="px-4 py-3">Status</th><th className="px-4 py-3">Updated</th><th className="px-4 py-3 text-right">Actions</th></tr></thead><tbody className="divide-y divide-border/65">{filtered.map((walk) => <tr key={walk.id} className="hover:bg-muted/25"><td className="px-4 py-3 font-mono text-xs font-medium">{walk.id}</td><td className="px-4 py-3"><p className="text-xs font-medium">{walk.plant}</p><p className="mt-0.5 text-[11px] text-muted-foreground">{walk.zone}</p></td><td className="px-4 py-3 text-xs">{walk.leadName}</td><td className="px-4 py-3 text-xs tabular-nums">{walk.participants.length}</td><td className="whitespace-nowrap px-4 py-3 text-xs">{formatShortDate(walk.scheduledDate)}</td><td className="px-4 py-3 text-xs tabular-nums">{walk.observationIds.length}</td><td className="px-4 py-3 text-xs tabular-nums">{walk.actionIds.length}</td><td className="px-4 py-3"><GembaStatusBadge status={walk.status} /></td><td className="whitespace-nowrap px-4 py-3 text-xs text-muted-foreground">{formatShortDate(walk.updatedAt)}</td><td className="px-4 py-3"><div className="flex justify-end gap-1">{walk.status === "Draft" && mayConduct ? <Button size="sm" variant="outline" onClick={() => onStart(walk)}><Play className="size-3.5" />Start</Button> : walk.status === "In Progress" && mayConduct ? <Button size="sm" nativeButton={false} render={<Link href={`/gemba/${walk.id}/walk`} />}><Footprints className="size-3.5" />Continue</Button> : <Button size="icon-sm" variant="ghost" nativeButton={false} render={<Link href={`/gemba/${walk.id}`} />} aria-label={`View ${walk.id}`}><Eye className="size-4" /></Button>}{walk.status === "Completed" && <Button size="icon-sm" variant="ghost" nativeButton={false} render={<Link href={`/gemba/${walk.id}/report`} />} aria-label={`Report for ${walk.id}`}><FileText className="size-4" /></Button>}</div></td></tr>)}</tbody></table></div>
    <div className="grid gap-2 p-3 lg:hidden">{filtered.map((walk) => <Link key={walk.id} href={walk.status === "In Progress" && mayConduct ? `/gemba/${walk.id}/walk` : `/gemba/${walk.id}`} className="rounded-lg border bg-background p-4 outline-none focus-visible:ring-2 focus-visible:ring-ring"><div className="flex items-start justify-between gap-3"><div className="min-w-0"><p className="font-mono text-xs font-semibold">{walk.id}</p><p className="mt-1 line-clamp-2 text-sm font-medium">{walk.purpose}</p></div><GembaStatusBadge status={walk.status} /></div><p className="mt-3 text-xs text-muted-foreground">{walk.plant} · {walk.zone} · {formatShortDate(walk.scheduledDate)}</p><div className="mt-3 flex gap-4 text-xs"><span><strong>{walk.observationIds.length}</strong> observations</span><span><strong>{walk.actionIds.length}</strong> actions</span></div></Link>)}</div>
