...
export function CreateLinkedActionDialog({ open, onOpenChange, context, onCreated }: {
  ...
  return <Dialog open={open} onOpenChange={onOpenChange}>
    <DialogContent className="max-w-xl">
      ...
      {context && <div className="grid gap-4">
        <div className="rounded-lg border bg-muted/25 p-3">
          <div className="flex flex-wrap items-center gap-2"><Badge variant={context.source === "Red Flag" ? "danger" : "info"}>{context.source}</Badge><span className="font-mono text-xs text-muted-foreground">{context.sourceId}</span>{context.sourceObservationId && <span className="font-mono text-xs text-muted-foreground">{context.sourceObservationId}</span>}{context.defaultPriority && <Badge variant={context.defaultPriority === "Critical" || context.defaultPriority === "High" ? "danger" : context.defaultPriority === "Medium" ? "warning" : "secondary"}>{context.defaultPriority} context</Badge>}</div>
        ...
        <div className="grid gap-4 sm:grid-cols-2">
          ...
          <Field label="Due Date"><Input type="date" value={dueDate} disabled /></Field>
          ...
          <Field label="Zone Leader"><Input value={zone?.leader ?? "—"} disabled /></Field>
          <Field label="Assigned By"><Input value={currentUser.name} disabled /></Field>
        </div>
      </div>}
      <DialogFooter><Button variant="outline" onClick={() => onOpenChange(false)}>Cancel</Button><Button disabled={!context || !category || !responsibleId || busy} onClick={submit}>{busy ? "Creating..." : "Create Action"}</Button></DialogFooter>
    </DialogContent>
  </Dialog>;
}
